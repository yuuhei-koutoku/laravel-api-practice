...
namespace Tests\Feature\Controllers\Larashop\API\MePurchasredProductController;
...
class GetPurchasedProductsTest extends TestCase
{
    ...
    public function test_get_purchased_products(): void
    {
        ...
        $response = $this->actingAs($seller)->getJson('/larashop/api/me/purchased_products');
        $response->assertStatus(200)
            ->assertJson(fn (AssertableJson $json) => $json->has('products', 0));

        $response = $this->actingAs($other)->getJson('/larashop/api/me/purchased_products');
        $response->assertStatus(200)
            ->assertJson(fn (AssertableJson $json) => $json->has('products', 0));
    }
}
