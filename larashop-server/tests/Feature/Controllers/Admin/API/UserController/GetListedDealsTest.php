...
class GetListedDealsTest extends TestCase
{
    use RefreshDatabase;
     ...
     * 正常系
     */
    public function test_get_listed_deals(): void
    {
        ...
        $response = $this->actingAs($this->adminUser, 'admin_api')->getJson("/admin/api/users/{$seller->id}/listed_deals");
        ...
        $response = $this->actingAs($this->adminUser, 'admin_api')->getJson("/admin/api/users/{$seller2->id}/listed_deals");
        $response->assertStatus(200)
            ->assertJsonCount(1, 'deals');

        $response = $this->actingAs($this->adminUser, 'admin_api')->getJson("/admin/api/users/{$buyer->id}/listed_deals");
        $response->assertStatus(200)
